package ru.sulgik.remotearduino.modules.permission.ui
...
import ru.sulgik.remotearduino.modules.permission.ui.BaseMaterialPermissionRequestFragment.Builder.Companion.EXTRA_PERMISIONS
import ru.sulgik.remotearduino.modules.permission.ui.BaseMaterialPermissionRequestFragment.Builder.Companion.EXTRA_TOKEN
import ru.sulgik.remotearduino.modules.permission.ui.BaseMaterialPermissionRequestFragment.Builder.Companion.TOKEN_NONE
import ru.sulgik.remotearduino.modules.permission.ui.BaseMaterialPermissionRequestFragment.Builder.Companion.buildIntent
import ru.sulgik.remotearduino.modules.permission.MaterialPermissionManager.Companion.RECEIVER
import ru.sulgik.remotearduino.modules.permission.PermissionRequestConfig
import ru.sulgik.remotearduino.modules.permission.asPathArray
import ru.sulgik.remotearduino.modules.permission.ui.def.DefaultPermissionRequestFragment
import ru.sulgik.remotearduino.modules.permission.rebuild

private typealias Config = PermissionRequestConfig.() -> Unit
...
abstract class BaseMaterialPermissionRequestFragment : BottomSheetDialogFragment() {
    ...
    fun request(permissions : Array<MaterialPermission>){
        requestPermissions(permissions.asPathArray(),
            PERMISSION_REQUEST
        )
    }
    ...
    fun sendCallback(permissions: Array<MaterialPermission>){
        ...
        LocalBroadcastManager.getInstance(requireActivity()).sendBroadcast(buildIntent(Intent(RECEIVER), permissions, token))
    }


    companion object{
        const val TAG = "ru.sulgik.remotearduino.modules.permission:BaseMaterialPermissionRequestFragment:456648"
    ...
    class Builder (){
        ...
        var config : PermissionRequestConfig =
            PermissionRequestConfig.build()

        fun setConfig(conf : PermissionRequestConfig): Builder {
            config = conf
            return this
        }
        fun setConfig(conf: Config): Builder {
            config =
                PermissionRequestConfig.build(
                    conf
                )
